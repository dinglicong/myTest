...
*/
BMapLib.SearchInRectangle = function(map ,opts){
if (!map) {
return;
}
...
// 在这里加一个属性，使用的时候设置页面上用于显示拉框右下角的坐标控件的id，
ShowPointLocalContorlName:"",
callback: $.noop
...
//TOFIX 再次使用筛选框的时候，去掉原内存中的数据....
try{
	if(BMapLib._toolInUse && OperationMask._maskElement) {
		this._isOpen = false;
...
//拉框选取完成，然后调用搜索 
// me.local.searchInBounds(me._searchWords,bds);//注释掉搜索功能
...
//把右下角坐标赋值给页面控件
me._opts.callback(northWestPoint, southEastPoint);
//$("#"+me._opts.ShowPointLocalContorlName+"").val(southEastPoint.lng + "," + southEastPoint.lat + ';' + northWestPoint.lng + "," + northWestPoint.lat);
// 停止事件冒泡传播和默认事件
stopBubble(e);
return preventDefault(e);
